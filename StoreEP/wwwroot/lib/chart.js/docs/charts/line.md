...
When the `data` array is an array of numbers, the x axis is generally a [categoria](../axes/cartesian/categoria.md#Category Axis). The points are placed onto the axis using their position in the array. When a line chart is created with a categoria axis, the `labels` property of the data object must be specified.
